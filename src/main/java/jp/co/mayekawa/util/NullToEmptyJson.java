...
/**
 * Jackson のツリーモデル ({@link JsonNode}) を再帰的に走査し、
 * {@code null}（および {@link com.fasterxml.jackson.databind.node.NullNode}）を
 * 空文字 {@code ""}（{@link TextNode}）へ置換するユーティリティ。
 *
 * <p>本ユーティリティは、レスポンスJSONで {@code null} を返さずに空文字として返したい、
 * というAPI仕様に対応するための「最終出力整形」を目的とする。</p>
 *
 * <h2>重要な注意</h2>
 * <ul>
 *   <li>この処理は JSON の型を変える可能性がある。
 *       例：本来数値/配列として扱われるプロパティが {@code null} の場合、{@code ""}（文字列）になる。</li>
 *   <li>対象の {@link JsonNode} を<strong>破壊的に更新</strong>する（ObjectNode/ArrayNode を直接 {@code set} で書き換える）。</li>
 *   <li>入力が {@code null} の場合も {@code ""} の {@link TextNode} を返す。</li>
 * </ul>
 *
 * <p>上記の通り、クライアント側が厳密な型を期待する場合は互換性問題になり得るため、
 * 利用範囲は「レスポンスの最終段」に限定することが望ましい。</p>
 */
public final class NullToEmptyJson {
     ...
     * <ul>
     *   <li>{@code null} または {@link JsonNode#isNull()} のノードは {@code ""}（{@link TextNode}）に置換する。</li>
